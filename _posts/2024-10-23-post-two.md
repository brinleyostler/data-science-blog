...
Whenever we as data scientists want to scrape data from the web, we have to ensure we are following best practice. Check that site's robots.txt file (accessible at the 'base url'/robots.txt). Good scraping practice ensures we are using delay functions in our code, only scraping necessary data, and not accessing sensitive or restricted information.
...
Here's a summary of the main steps I followed for my data collection. For a full document of my code, check out my GitHub [best-selling-books](https://github.com/brinleyostler/best-selling-books) repository.
